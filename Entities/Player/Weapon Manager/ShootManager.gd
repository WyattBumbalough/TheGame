...
func shoot():
	if animations_manager.anims.is_playing():
		return
	if !cw.is_shooting and !cw.is_reloading and cw.current_ammo > 0:
		cw.is_shooting = true
		weapon_manager.weapon_fired.emit()
		if cw.shoot_anim_name != "":
			...
			weapon_manager.weapon_sound_player(cw.shoot_sound, cw.shoot_sound_speed, true)
		else:
			printerr("%s doesn't have a shoot animation." % cw.weapon_name)
			
		if weapon_manager.infinite_ammo == false:
			cw.current_ammo -= 1
